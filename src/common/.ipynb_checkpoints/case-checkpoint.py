...
class CheckResult:
    """
    Resultado de un check de QA individual.

    Attributes
    ----------
    name : str
        Nombre corto del check (p.ej. "PTV volume").
    passed : bool
        True si el check se considera pasado.
    score : float
        Puntuación del check (típicamente 0–1, pero puedes usar 0–100
        si prefieres; el agregador se encarga).
    message : str
        Mensaje principal legible para el usuario.
    details : Dict[str, Any]
        Información extra para debugging o para reportes detallados.
    """
    name: str
    passed: bool
    score: float
