...
# ============================================================
# 0) CONFIGURACIÓN GLOBAL PARA UI / INTERFAZ
#    - Pesos por sección (CT / Structures / Plan / Dose / Other)
#    - Encendido/apagado de secciones
#    - Encendido/apagado y peso por check individual
#    - Nombre visible de cada check (para el reporte/interfaz)
# ============================================================

# --- 0.1) Secciones generales ---

GLOBAL_SECTION_CONFIG: Dict[str, Dict[str, Any]] = {
    # Pesos relativos de cada bloque grande (se usarán a nivel UI
    # o en un agregador más arriba). Puedes tunearlos a gusto.
    "CT": {
        "label": "CT",
        "enabled": True,
        "weight": 0.25,
...
def get_global_section_config() -> Dict[str, Dict[str, Any]]:
    """
...
# --- 0.2) Checks individuales por sección ---

# Estructura:
# GLOBAL_CHECK_CONFIG[section][check_key] = {
#     "result_name": "<CheckResult.name visible>",
#     "enabled": True/False,
#     "weight": float,
#     "description": "Texto corto para UI (opcional)",
# }
#
# NOTA:
# - result_name debe coincidir con el .name del CheckResult para que
#   el agregador pueda mapear pesos correctamente.
# - section debe coincidir con los groups usados en REPORTING_CONFIG
#   ("CT", "Structures", "Plan", "Dose", "Other").

GLOBAL_CHECK_CONFIG: Dict[str, Dict[str, Dict[str, Any]]] = {
    # ----------------------
    # CT
    # ----------------------
    "CT": {
    ...
    "Structures": {
        ...
        "LATERALITY": {
            # Ojo: mantengo el nombre con la "D" inicial para no romper nada
    ...
    # ----------------------
    # Other (por si en el futuro añades cosas sueltas)
    # ----------------------
    "Other": {
        # Ejemplo futuro:
        # "QA_LOGS": {
...
def get_global_check_config() -> Dict[str, Dict[str, Dict[str, Any]]]:
    """
...
#      derivada de GLOBAL_CHECK_CONFIG
# ------------------------------------------------------------

# Estructura final (igual que antes):
...
# }
#
# Pero ahora los pesos y qué checks están activos se leen de:
#   - GLOBAL_SECTION_CONFIG
#   - GLOBAL_CHECK_CONFIG
#
# Esto deja el archivo listo para una interfaz de personalización,
...
def _build_default_aggregate_from_global_checks() -> Dict[str, Any]:
    ...
    for section_name, checks in GLOBAL_CHECK_CONFIG.items():
        # Si toda la sección está apagada, ignoramos todos sus checks
        ...
        for check_key, cfg in checks.items():
            if not cfg.get("enabled", True):
                # Check individual apagado → sin peso
                continue
    ...
    # Si quieres, podrías normalizar aquí los pesos.
    return {
...
AGGREGATE_SCORING_CONFIG: Dict[str, Dict[str, Any]] = {
    # De momento sólo perfil DEFAULT, pero lo dejamos preparado por sitio
...
def get_aggregate_scoring_config(site: str | None = None) -> Dict[str, Any]:
    ...
    IMPORTANTE:
    - Los pesos se definen en GLOBAL_CHECK_CONFIG.
    - El encendido/apagado de secciones y checks también se define ahí.
    """
